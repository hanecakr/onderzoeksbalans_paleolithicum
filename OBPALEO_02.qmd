...
Misschien is het nuttig om het paleolithisch onderzoek in Vlaanderen eerst wat te kaderen binnen het algemene steentijdonderzoek in Belgische context (@fig-005). Het aantal terreincampagnes op sites met een paleolithische of neolithische component wordt sterk beïnvloed door het grottenonderzoek in het zuiden van het land. Bij de sites met een mesolithisch component domineert het onderzoek op Vlaamse bodem.
...
Onderstaande grafiek (@fig-a) is aangemaakt o.b.v. een excel-bestand, met de achterliggende numerieke gegevens.
Deze grafiek is (ongeveer) hetzelfde als @fig-005.
...
df <- readxl::read_xlsx("OBPALEO_terreincampagnes.xlsx") |> 
     mutate(regio = fct_rev(fct_inorder(regio)),
            periode = fct_inorder(periode))

ggplot(df) +
     geom_col(aes(x=periode, y=n, fill=regio)) +
     geom_text(aes(x=periode, y=n, label=n), position = position_stack(vjust = .50)) +
     scale_fill_manual(values = c("tomato3", "#ffd400")) +
     xlab("") +
     theme_minimal() +
...
Wanneer enkel naar het steentijdonderzoek in Vlaanderen wordt gekeken, en we dit binnen de grote periodes opsplitsen in subfases (vroeg-, midden-, laat-, ...), komt hieruit wel wat variatie naar voren, (@fig-007). Algemeen gezien valt een sterke stijging op in het aantal sites sinds de opkomst van de
